...
class SpringCoverageTest {
  ...
  @Test
  fun `spring bean throws when named bean lookup fails`() {
    val context = mockk<ApplicationContext>()
    every { context.getBean("missing", NamedService::class.java) } throws IllegalStateException("missing")
    SpringContextUtil.context = context

    assertThatThrownBy { SpringBean(NamedService::class.java, "missing").value }
      .isInstanceOf(IllegalStateException::class.java)
  }
  ...
  @Test
  fun `mountAnnotatedPages mounts annotated pages`() {
